/*
 * services/honalab/greenlight.js
 * AUTHOR : beryh
 * DATE : 2014. 2. 20
 * GRENNLIGHT COLLECTION
 */
...
exports.set = function(req, res) {
  ...
  if (!req.params.id && (req.body.title === null || req.body.content === null)) {
...
exports.get = function(req, res) {
  if (req.params.id) {
    ...
    db.articles.find('greenlight', req.params.id, function(err, info) {
      if (err) {
        return res.json(500, {'message': err});
      }
...
exports.del = function(req, res) {
  ...
  db.articles.delete('greenlight', req.params.id, function(err, info) {
    if (err) {
      return res.json(500, {'message': err});
    }
...
exports.addLike = function(req, res) {
  ...
  db.articles.find('greenlight', req.params.id, function(err, info) {
    if (info.like && (info.like.indexOf(req.user.id) !== -1)) {
      ...
      db.articles.addLike('greenlight', req.params.id, req.user.id, function(_err, _info) {
        if (_err) {
...
exports.delLike = function(req, res) {
  ...
  db.articles.find('greenlight', req.params.id, function(err, info) {
    if (!info.like || info.like.indexOf(req.user.id) === -1) {
      ...
      db.articles.delLike('greenlight', req.params.id, req.user.id, function(_err, _info) {
        if (_err) {
          return res.json(500, {'message': _err});
        }
